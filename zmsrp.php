...
// constructor
function srp_plugin()     {
parent::__construct(false, $name = __('SidePost', 'wp_widget_plugin') );
}
...
if( $instance) {
	...
	$srp_th = esc_attr($instance['srp_th']);
	$srp_ex = esc_attr($instance['srp_ex']);
	$srp_tt = esc_attr($instance['srp_tt']);
} else {
	$title = '';
	$srp_ct = '';
	$srp_pp = '';
...
?>
	...
	<p><input class="upisati" id="<?php echo $this->get_field_id('srp_br'); ?>" name="<?php echo $this->get_field_name('srp_br'); ?>" type="number" value="<?php echo $srp_br; ?>" />
		<label for="<?php echo $this->get_field_id('srp_br'); ?>"><?php _e(' Number of words in excerpt.', 'wp_widget_plugin'); ?></label></p>
	<p><input id="<?php echo $this->get_field_id('srp_th'); ?>" name="<?php echo $this->get_field_name('srp_th'); ?>" type="checkbox" value="1" <?php checked( '1', $srp_th ); ?> />
		<label for="<?php echo $this->get_field_id('srp_th'); ?>"><?php _e('Show thumbnail.', 'wp_widget_plugin'); ?></label></p>
	<p><input id="<?php echo $this->get_field_id('srp_tt'); ?>" name="<?php echo $this->get_field_name('srp_tt'); ?>" type="checkbox" value="1" <?php checked( '1', $srp_tt ); ?> />
		<label for="<?php echo $this->get_field_id('srp_tt'); ?>"><?php _e('Show title.', 'wp_widget_plugin'); ?></label></p>
	<p><input id="<?php echo $this->get_field_id('srp_dv'); ?>" name="<?php echo $this->get_field_name('srp_dv'); ?>" type="checkbox" value="1" <?php checked( '1', $srp_dv ); ?> />
		<label for="<?php echo $this->get_field_id('srp_dv'); ?>"><?php _e('Show date.', 'wp_widget_plugin'); ?></label></p>
	<p><input id="<?php echo $this->get_field_id('srp_ex'); ?>" name="<?php echo $this->get_field_name('srp_ex'); ?>" type="checkbox" value="1" <?php checked( '1', $srp_ex ); ?> />
		<label for="<?php echo $this->get_field_id('srp_ex'); ?>"><?php _e('Show excerpt.', 'wp_widget_plugin'); ?></label></p>
	<p>*Here is the list of categories and ID numbers:
 <?php //category list
...
// Fields
	...
	$instance['srp_th'] = strip_tags($new_instance['srp_th']);
	$instance['srp_ex'] = strip_tags($new_instance['srp_ex']);
	$instance['srp_tt'] = strip_tags($new_instance['srp_tt']);
return $instance;
}
...
// these are the widget options
	...
	$srp_th = $instance['srp_th'];
	$srp_ex = $instance['srp_ex'];
	$srp_tt = $instance['srp_tt'];
echo $before_widget;
// Display the widget
...
<ul><li>
		<?php if ($srp_tt) {?>
		<div class="naslov"> <a href="<?php the_permalink()?>" rel="bookmark"><?php the_title()?></a></div>
		<?php } ?>

		<?php if ($srp_dv){	echo "<div class='datum'>";	$dta = the_time(get_option('date_format'));	echo $dta;	echo "</div>";}?>
